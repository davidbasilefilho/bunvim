...
export const isNormal = (mode: Mode): mode is { type: "normal" } =>
	mode.type === "normal";

export const isInsert = (mode: Mode): mode is { type: "insert" } =>
	mode.type === "insert";

export const isVisual = (
	mode: Mode,
): mode is { type: "visual"; subtype: "char" | "line" | "block" } =>
	mode.type === "visual";

export const isCommand = (
	mode: Mode,
): mode is { type: "command"; input: string } => mode.type === "command";
...
export const cursorIsBlock = (mode: Mode): boolean => {
	if (
		mode.type === "insert" ||
		mode.type === "command" ||
		mode.type === "search"
	) {
		return Options.opt.cursorStyle === "block";
	}
	return true;
};
