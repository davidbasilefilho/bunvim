...
function targetToBunTarget(platform: Platform, arch: Arch, baseline: boolean): CompileTarget {
...
function targetToOutputName(platform: Platform, arch: Arch, baseline: boolean): string {
...
function parseTargetString(input: string): Effect.Effect<TargetInput, Error> {
  const match = input.match(/^(linux|darwin|windows)-(x64|arm64)(?:-baseline)?$/);
  if (!match) {
    return Effect.fail(
      new Error(
...
function validateAndTransform(target: TargetInput): TargetOutput {
  const bunTarget = targetToBunTarget(target.platform, target.arch, target.baseline);
...
> {
  ...
  if (args.includes("all")) {
    const platforms: [Platform, Platform, Platform] = ["linux", "darwin", "windows"];
  ...
  const targetStrings = args.slice(targetArgIndex + 1).filter((a) => !a.startsWith("-"));
