...
export function EditorBuffer(props: EditorBufferProps) {
  ...
  const isInVisualSelection = (lineNum: number, col: number): boolean => {
    ...
    if (lineNum === startLine) {
      return (
        col >=
        (props.visualAnchorLine < props.cursorLine ? props.visualAnchorColumn : props.cursorColumn)
      );
    }
    if (lineNum === endLine) {
      return (
            ...
            <box flexDirection="row" key={lineNum}>
              ...
              <box flexGrow={1} flexDirection="row">
                <For each={line.split("")}>
                  {(char, colIdx) => {
                    const isCursor =
                      isCurrentLine && colIdx() === props.cursorColumn && props.isActive;
                    const inVisual =
                      props.mode.type === "visual" && isInVisualSelection(lineNum, colIdx());
                    const bg = isCursor
                      ? colors().cursor
                      : inVisual
                        ? colors().selection
