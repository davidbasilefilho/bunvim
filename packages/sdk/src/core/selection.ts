...
export const isCollapsed = (sel: Selection): boolean => positionEquals(sel.anchor, sel.head);
...
export const isForward = (sel: Selection): boolean => positionCompare(sel.anchor, sel.head) <= 0;

export const isBackward = (sel: Selection): boolean => positionCompare(sel.anchor, sel.head) > 0;

export const extendTo = (sel: Selection, newHead: Position): Selection =>
  selection(sel.anchor, newHead);

export const collapseToHead = (sel: Selection): Selection => selection(sel.head, sel.head);

export const collapseToAnchor = (sel: Selection): Selection => selection(sel.anchor, sel.anchor);
...
export const flip = (sel: Selection): Selection => selection(sel.head, sel.anchor);

export const translate = (sel: Selection, deltaLine: number, deltaColumn: number): Selection =>
  selection(
...
export const moveTo = (_sel: Selection, newPos: Position): Selection => selection(newPos, newPos);
