...
export function undo(buffer: Buffer.BufferState): Buffer.BufferState | undefined {
  ...
  for (let i = current.edits.length - 1; i >= 0; i--) {
    ...
    if (edit.type === "insert") {
      ...
      newState = Buffer.deleteInRange(newState, { start: edit.pos, end }) ?? newState;
    } else {
      newState = Buffer.insertAt(newState, edit.range.start, edit.text) ?? newState;
    }
  }
...
export function redo(buffer: Buffer.BufferState): Buffer.BufferState | undefined {
  const current = nodes.get(currentNodeId);
