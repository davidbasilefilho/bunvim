...
export function createState(content: string, props: Partial<BufferProps> = {}): BufferState {
...
export function getBufferForFile(path: string): BufferState | undefined {
  ...
  return id !== undefined ? bufferStore.buffers.find((b) => b.id === id) : undefined;
}

export function getOrCreateFileBuffer(path: string, content: string): BufferState {
...
export function getLine(state: BufferState, lineNumber: number): string | undefined {
  return Rope.getLine(state.rope, lineNumber);
}

export function getLineLength(state: BufferState, lineNumber: number): number | undefined {
...
export function getTextInRange(state: BufferState, range: Range): string | undefined {
...
export function offsetToPosition(state: BufferState, offset: number): Position | undefined {
  return Rope.offsetToPosition(state.rope, offset);
}

export function positionToOffset(state: BufferState, pos: Position): number | undefined {
  return Rope.positionToOffset(state.rope, pos);
}

export function insertAt(state: BufferState, pos: Position, text: string): BufferState | undefined {
...
export function deleteInRange(state: BufferState, range: Range): BufferState | undefined {
  const startOffset = Rope.positionToOffset(state.rope, range.start);
  const endOffset = Rope.positionToOffset(state.rope, range.end);
  if (startOffset === undefined || endOffset === undefined) {
    return undefined;
