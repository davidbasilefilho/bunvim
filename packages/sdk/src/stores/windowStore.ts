...
import type { WindowId, WindowState } from "./windowTypes";
...
export function setCursor(id: WindowId, line: number, column: number): WindowState | undefined {
...
export function setBuffer(id: WindowId, bufId: number): WindowState | undefined {
  return update(id, (win) => {
    const newBufferIds = win.bufferIds.includes(bufId) ? win.bufferIds : [...win.bufferIds, bufId];
...
export function moveFocus(direction: "h" | "j" | "k" | "l"): void {
  const currentIdx = windowStore.windows.findIndex((w) => w.id === windowStore.activeWindowId);
  if (currentIdx === -1) return;

  const nextIdx = resolveDirectionIndex(currentIdx, direction, windowStore.windows.length);
    ...
    windowStore.windows.map((w) => {
      if (w.id === currentWin.id) {
        return {
          ...w,
          bufId: nextBufId,
          bufferIds: w.bufferIds.includes(nextBufId) ? w.bufferIds : [...w.bufferIds, nextBufId],
        };
      }
      if (w.id === nextWin.id) {
        return {
