...
/**
 * Strip ANSI escape sequences from a string. Matches sequences starting with ESC (\x1b) followed by
 * control sequences.
 */
const stripAnsiSequences = (str: string): string => {
  const ansiPattern = /\x1b\[[\d;]*[A-Za-z]|\x1b\][\d;]*\x07|\x1b\[[\d;]*m|\x1b\[?[\d;]*[A-Za-z]/g;
  return str.replace(ansiPattern, "");
};

/**
 * Get the offset in the string for a given line and column. Handles UTF-16 surrogate pairs
 * correctly.
 */
const getOffsetAtPosition = (text: string, line: number, column: number): number => {
...
/** Get line and column from a string offset. Handles UTF-16 surrogate pairs correctly. */
const getPositionAtOffset = (text: string, offset: number): { line: number; column: number } => {
...
/**
 * Process a text input action and return new state. Returns null if the action cannot be performed
 * (e.g., backspace at start).
 */
...
/**
 * Normalize pasted content - convert bytes to string, strip ANSI sequences. Used when handling
 * paste events.
 */
