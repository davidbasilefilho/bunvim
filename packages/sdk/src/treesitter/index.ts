...
const languageConfigs: Record<string, { wasm: string; queries: string[] }> = {
  ...
  cpp: {
    ...
    queries: [`${QUERIES_BASE_URL}/c/highlights.scm`, `${QUERIES_BASE_URL}/cpp/highlights.scm`],
  },
  c: {
...
const getClient = (): Effect.Effect<TreeSitterClient | null, TreesitterError> =>
  Effect.gen(function* () {
    if (client) return client;
    const newClient = yield* Effect.try({
      try: () =>
        ...
        new TreesitterError({
          message: "Failed to create TreeSitter client",
          cause: e,
        }),
    });
    yield* Effect.tryPromise({
      try: () => newClient.initialize(),
      catch: (e) =>
        new TreesitterError({
          message: "Failed to initialize TreeSitter client",
          cause: e,
        }),
    });
    client = newClient;
...
export const getHighlights = (
  content: string,
  language: string,
): Effect.Effect<HighlightRange[], TreesitterError> =>
  Effect.gen(function* () {
    const tsClient = yield* getClient();

    if (!tsClient) {
      const empty: HighlightRange[] = [];
      return empty;
    }
