...
const logDir = process.env.XDG_CACHE_HOME || path.join(process.env.HOME || "", ".cache", "bvim");
...
const formatLogEntry = (level: string, message: string, timestamp: Date): string => {
...
const writeLogEntry = (level: string, message: string, data?: unknown): void => {
  ensureLogDir();
...
export const logSync = {
  info: (message: string, data?: unknown): void => writeLogEntry("INFO", message, data),
  warn: (message: string, data?: unknown): void => writeLogEntry("WARN", message, data),
  error: (message: string, data?: unknown): void => writeLogEntry("ERROR", message, data),
  debug: (message: string, data?: unknown): void => writeLogEntry("DEBUG", message, data),
};
