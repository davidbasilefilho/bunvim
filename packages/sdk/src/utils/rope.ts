...
export const getLine = (rope: Rope, lineNumber: number): string | undefined => {
  ...
  const end = nextLineStart !== undefined ? nextLineStart - 1 : rope.content.length;
...
export const getLineLength = (rope: Rope, lineNumber: number): number | undefined => {
...
export const offsetToPosition = (rope: Rope, offset: number): Position | undefined => {
...
export const positionToOffset = (rope: Rope, pos: Position): number | undefined => {
  ...
  const lineEnd = nextLineStart !== undefined ? nextLineStart : rope.content.length;
...
export const insert = (rope: Rope, offset: number, text: string): Rope => {
  ...
  const newContent = rope.content.slice(0, offset) + text + rope.content.slice(offset);
...
export const replace = (rope: Rope, start: number, end: number, text: string): Rope => {
  if (start < 0 || end > rope.content.length || start > end) {
    return rope;
  }

  const newContent = rope.content.slice(0, start) + text + rope.content.slice(end);
  return fromString(newContent);
};
