 ...
 *
 * OpenTUI Solid's reconciler throws "Orphan text error" when a text node (created by SolidJS
 * reconciliation) is inserted as a direct child of a non-text element (like <box>). This happens
 * during SolidJS reconciliation when dynamic expressions or <Show> transitions produce empty text
 * nodes.
 *
 * This patch makes the orphan text check non-fatal for empty text nodes, skipping insertion instead
 * of crashing. Non-empty orphan text nodes still throw (that's a real layout bug).
 *
...
const possiblePaths = [resolve(bunDir, "node_modules", ".bun")];
...
if (!patched) {
  console.log(
    "[patch] No @opentui/solid index.js found to patch. This may be normal if the package isn't installed yet.",
  );
}
